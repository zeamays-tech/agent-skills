...
## Local preparation before remote publication

The release flow prepares a local version commit and annotated tag after local checks and confirmation, before GitHub authentication, fetch, or push. Remote failure retains both for a same-version retry. Cached remote default-branch and tag information governs local preparation; fresh remote checks still gate publication. Dry runs validate only the local plan and do not establish remote readiness.
